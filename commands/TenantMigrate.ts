...
export default class TenantMigrate extends BaseCommand {
    ...
    public static commandName = 'tenant:migrate'
    public static description = 'Custom command to migrate database single or multiple tenants.'
    ...
    * */
    private async migrate(tenant){
        ...
        await migrator.run()
        this.logger.info(`Migrating tenant #${tenant.id} (${tenant.database})`)
        for (const migratedFile in migrator.migratedFiles) {
            const status = migrator.migratedFiles[migratedFile].status === 'completed' ? 'migrated' : migrator.migratedFiles[migratedFile].status
